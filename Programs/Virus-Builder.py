...
try:
    ...
    builder = ctk.CTk()
    builder.title(f"Azure{version_tool} - Virus Builder")
    builder.geometry("800x600")
    builder.resizable(False, False)
    builder.configure(fg_color=colors["background"])
    builder.iconbitmap(os.path.join(tool_path, "Img", "azure.ico"))
    ...
    def ErrorLogs(message):
        ...
        messagebox.showerror(f"Azure {version_tool} - Virus Builder", message)

    def InfoLogs(message):
        messagebox.showinfo(f"Azure {version_tool} - Virus Builder", message)
