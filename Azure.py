...
{RED}
{RED}                                            _     ______   _ ____  _____ 
{RED}                                           / \   |__  / | | |  _ \| ____|
{RED}                                          / _ \    / /| | | | |_) |  _|  
{RED}                                         / ___ \  / /_| |_| |  _ <| |___ 
{RED}                                        /_/   \_\/____|\___/|_| \_\_____|
{RESET}
{RED}                                       {github_tool}
{menu}"""
...
while True:
   try:
      ...
      choice = input(MainColor(f""" ┌──({white}{username_pc}@Azure)─{red}[{white}~/{os_name}/Menu-{menu_number}]
 └─{white}$ {reset}"""))
      ...
      if choice in ['N', 'n', 'NEXT', 'Next', 'next']:
         ...
         if os_name == "Linux":
            ...
            messagebox.showinfo(f"Azure {version_tool} - Virus Builder", "The builder virus is only compatible with Windows, under Linux it can encounter big problems.")
         ...
         messagebox.showinfo(f"Azure {version_tool} - Virus Builder", "It is important to disable your antivirus (Real-time Protection) before building, so that no files are deleted.")
         try:
            zip_file_path = os.path.join(tool_path, "Program", "FileDetectedByAntivirus", "Password(azure).zip")
            ...
            if not os.path.exists(file_path):
               if os.path.exists(zip_file_path):
                  print(f"{BEFORE + current_time_hour() + AFTER} {INFO} Decompression of the encrypted file: {white}Program\\FileDetectedByAntivirus\\Password(azure).zip")
